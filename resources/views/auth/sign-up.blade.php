...
@section('content')
    <x-forms.auth-forms title="Регистрация" action="" method="POST">
        @csrf
        <x-forms.text-input name="name" type="text" placeholder="Name" required="true" value="{{ old('name') }}" />
        ...
        <x-slot:buttons>
            <div class="space-y-3 mt-5">
                <div class="text-xxs md:text-xs"><a href="{{ route('auth.login') }}" class="text-white hover:text-white/70 font-bold">Войти в аккаунт</a></div>
            </div>
        </x-slot:buttons>
    </x-forms.auth-forms>
@endsection
